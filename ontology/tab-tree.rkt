...
(define-catch (get-item line #:symbol (symbol "\t") #:hash-tab-tree (hash-tab-tree #f))
  (let* ((res-name (if hash-tab-tree
                      #f
      ...
      (if res-name
        (hash-insert res (cons '_name (nth (nth res-name 1) 3)))
        res)))
...
(define-catch (fill-tree-iter source-lines result-tree #:hash-tab-tree (hash-tab-tree #f))
  (cond
    ((empty? source-lines) result-tree)
    (else
      (let* ((line (car source-lines))
            (level (count-tabs line))
            (item (get-item line #:hash-tab-tree hash-tab-tree))
            (item (->symbol item))
            (new-result-tree (if (hash-empty? item) (add-item-to-tree result-tree item level)))
        (fill-tree-iter (cdr source-lines) new-result-tree #:hash-tab-tree hash-tab-tree)))))
...
(define-catch (parse-tab-tree tree-file #:ontology? (ontology? #f) #:rules (rules #f) #:hash-tab-tree (hash-tab-tree #f))
  (let* (
    ...
    (if ontology?
      ...
      (fill-tree-iter tree-lines (list) #:hash-tab-tree hash-tab-tree))))
